...
const Login = () => {
  ...
  //Select the login state from the store.

  const store = useSelector((store) => store?.user);

  const { appError, serverError, userAuth, loading } = store;
  //--------- Formik -------
  ...
  useEffect(() => {
    if (userAuth) {
      ...
      if (userAuth.isAdmin) {
        // Navigate to the admin profile endpoint
        navigate(`/admin-profile/${userAuth._id}`);
      } else {
        // Navigate to the regular user profile endpoint
  ...
  return (
    <>
      <section className="min-h-screen relative py-20 2xl:py-40 bg-gray-900 overflow-hidden">
        <div className="absolute top-0 left-0 lg:bottom-0 h-full lg:h-auto w-full lg:w-4/12  lg:overflow-hidden ml-16">
          <img
            className="hidden lg:block h-[62.4%] w-screen object-cover float-right mt-[31.5%]"
            src={poster}
            alt=""
          />
        </div>
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex flex-wrap items-center -mx-4">
                  ...
                  <form onSubmit={formik.handleSubmit}>
                    <div className="flex flex-col mb-10">
                      ...
                      {appError || serverError ? (
                        <span className="text-red-400 font-bold">{`${serverError}: ${appError}`}</span>
                      ) : null}
                    </div>
                    {/* display err */}
                    ...
                    {loading ? (
                      <button
                        disabled
                        className="py-4 w-full bg-red-500  text-white font-bold rounded-full transition duration-200"
                      >
                        Login...
                      </button>
                    ) : (
              ...
              <div className="w-full lg:w-3/5 px-4 mb-16 lg:mb-0 order-first lg:order-last">
                <span className="flex mb-10 mx-auto items-center justify-center h-20 w-20 bg-white rounded-lg">
